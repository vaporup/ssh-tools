...
test_commands+=( "sshpass -e ${SCRIPT_PATH}/ssh-diff /etc/hosts ${TEST_SSH_USER}@${TEST_SSH_SERVER}"    )
test_commands+=( "sshpass -e ${SCRIPT_PATH}/ssh-diff -y /etc/hosts ${TEST_SSH_USER}@${TEST_SSH_SERVER}" )
test_commands+=( "sshpass -e ${SCRIPT_PATH}/ssh-diff -u /etc/hosts ${TEST_SSH_USER}@${TEST_SSH_SERVER}" )
...
test_commands+=( "sshpass -e ${SCRIPT_PATH}/ssh-ping -v -c 3 -D ${TEST_SSH_USER}@${TEST_SSH_SERVER}"    )
test_commands+=( "sshpass -e ${SCRIPT_PATH}/ssh-ping -v -c 3 -H ${TEST_SSH_USER}@${TEST_SSH_SERVER}"    )
